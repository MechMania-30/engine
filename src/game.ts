...
import { HelloWorldRequest, PlaneSelectRequest, PlaneSelectResponse, Player } from "./player"
...
export default class Game {
    ...
    async createAndStorePlanes(player0Response: PlaneSelectResponse, player1Response: PlaneSelectResponse) {
        ...
        function _createPlanes(playerPlanes: Plane[], type: PlaneType, count: number) {
    ...
    async runTurn() {
        ...
        if (this.turn == 0) {
            const player0PlanesSelectedResponse = await this.player0.getPlanesSelected(planeRequest);
            const player1PlanesSelectedResponse = await this.player1.getPlanesSelected(planeRequest);

            console.log(`Player 0 selected planes: ${player0PlanesSelectedResponse}`);
            console.log(`Player 1 selected planes: ${player1PlanesSelectedResponse}`);
        }
